...
import com.casestudy.Orders.Models.OrderForPaymentInput;
...
//	List<String> p5=new ArrayList<>();
	...
	@Override
	public OrderForPaymentOutput addNewOrder(OrderForInput order1) {
		OrderForPaymentOutput od = new OrderForPaymentOutput();
		Set<String> s1 = new HashSet<>();
		int total = 0;
		List<Products> p2 = new ArrayList<>();
		...
		for (String p : order1.getPID()) {
			ResponseEntity<Products> p1 = restTemplate.getForEntity("http://localhost:8085/product/getone/" + p,
					Products.class);
	...
	@Override
	public String updatePaymentStatus(String orderId) {
		Orders o1 = orderDatabase.findById(orderId).orElse(null);
...
//	
	  ...
	  String CustName=restTemplate.getForObject("http://localhost:8081/customer/customerFName/"+o1.getCUSTID(), String.class); // orderForOutput.setDealiiveryName(CustName);
	...
	public OrderForAdmin sendOrders() {
		// TODO Auto-generated method stub
		List<Orders> orderList = orderDatabase.findAll();
		orderForAdmin.setOrderList(orderList);
